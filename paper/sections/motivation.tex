...
\begin{tikzpicture}[
    ...
    \node (worker) [lens, right=of pm] {\textbf{Worker Lens}\\\scriptsize $\mathcal{M}_{\text{W}}$};
    \node (reviewer) [lens, below=of worker, yshift=-0.5cm] {\textbf{Reviewer Lens}\\\scriptsize $\mathcal{M}_{\text{R}}$};
    \node (verify) [state, left=of reviewer, xshift=-0.2cm] {\textbf{State\_VERIFY}\\\scriptsize Execution Harness};
...
\begin{tikzpicture}
    ...
    \filldraw[color=red] (2.4,-1.2) circle (3pt)
        node[right, font=\scriptsize, color=red!90!black, xshift=0.05cm, align=left]
        {\textbf{Sub-Centroid} ($C_{\text{sub}}$)\\\scriptsize Trace-isolated target};
    ...
    \node[color=red!80!black, font=\scriptsize, align=center] at (1.8,-2.4)
