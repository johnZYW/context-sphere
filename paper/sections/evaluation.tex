...
Table~\ref{tab:efficiency-profile} reports the corresponding local efficiency profile reconstructed from the per-case ledgers. Context Sphere spends slightly more per issue than the dense Vector baseline because it adds structural neighborhood evidence, but it yields the lowest estimated cost per successful local pass among the three retrieval configurations in this run.

\begin{table}[t]
\centering
\caption{Local efficiency profile reconstructed from per-case token and cost ledgers. Costs are estimates under the configured DeepSeek/MiniMax routing profile, not official benchmark fees.}
\label{tab:efficiency-profile}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Retrieval Configuration} & \textbf{Local Passes} & \textbf{LLM Calls} & \textbf{Total Cost} & \textbf{Cost / Pass} \\
\midrule
Dense Vector baseline & 36 & 491 & \$7.3113 & \$0.2031 \\
Context Sphere & \textbf{41} & 484 & \$8.2022 & \textbf{\$0.2001} \\
Hybrid & 39 & 457 & \$8.2289 & \$0.2110 \\
\bottomrule
\end{tabular}%
}
\end{table}
